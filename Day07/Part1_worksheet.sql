-- had to replace the regex version because it couldn't handle the larger string
...
-- easiest to reuse example code for part1 code
...
select min(item) from day07_part1_v1;
select max(item) from day07_part1_v1;

with minmax as (select min(item) minvalue, max(item) maxvalue from day07_part1_v1)
select * from minmax;
...
select position, item from t, day07_part1_v1;
